...
def execute_polygon_check():
    try:
        ...
        if summa <= 0:
            ...
            return (
                False,
                f"Polygons amount exceeds {MAX_RANGE_THRESHOLD}. Please reduce it to be able to export",
            )

        elif WARNING_RANGE_THRESHOLD < summa < MAX_RANGE_THRESHOLD:
            return (
                True,
                f"Polygons amount in range [{WARNING_RANGE_THRESHOLD}: {MAX_RANGE_THRESHOLD}]. It's acceptable",
            )

        elif 0 <= summa <= WARNING_RANGE_THRESHOLD:
            return (
                True,
                f"Polygons amount is less than {WARNING_RANGE_THRESHOLD}. It's acceptable",
            )
