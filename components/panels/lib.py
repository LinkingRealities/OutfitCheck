...
from components.constants import (ACCESSORIES_TRIANGLE_LIMIT,
                                  BOTTOM_TRIANGLE_LIMIT, MAX_TEXTURE_SIZE,
                                  OUTFIT_TRIANGLE_LIMIT, SHOES_TRIANGLE_LIMIT,
                                  TOP_TRIANGLE_LIMIT, var_bodyparts,
                                  var_forbiddentextures, var_garmenttypes)
...
def check_material_name(var_target, col):
    ...
    for count_m, _ in enumerate(slots_materials):
        if count_m > 0:
            if not bpy.data.objects[var_target].material_slots[
                count_m
            ].name == var_target + str(count_m):
                raise ValueError("Error appeared in materials")
        else:
            if (
                not bpy.data.objects[var_target].material_slots[count_m].name
                == var_target
            ):
                raise ValueError("Error appeared in materials")
...
def select_top_if_not_selected(box, panel_props):
    if check_existence(obj_name=var_garmenttypes["var_top"]):
        try:
            ...
            box.scale_y = 0.9
            box.label(
                text=f"{var_garmenttypes['var_top']} has an incorrect material name"
            )
            row = box.row()
...
def select_bottom_if_not_selected(box, panel_props):
    if check_existence(obj_name=var_garmenttypes["var_bottom"]):
        try:
            ...
            box.scale_y = 0.9
            box.label(
                text=f"{var_garmenttypes['var_bottom']} has an incorrect material name"
            )
            row = box.row()
...
def select_shoes_if_not_selected(box, panel_props):
    if check_existence(obj_name=var_garmenttypes["var_shoes"]):
        try:
            ...
            box.scale_y = 0.9
            box.label(
                text=f"{var_garmenttypes['var_shoes']} has an incorrect material name"
            )
            row = box.row()
...
def select_accessories_if_not_selectedv(box, panel_props):
    ...
    if check_existence(obj_name=var_garmenttypes["var_acs"]):
        try:
            ...
            box.scale_y = 0.9
            box.label(
                text=f"{var_garmenttypes['var_acs']} has an incorrect material name"
            )
            row = box.row()
...
def source_validation(layout):
    ...
    if textures_validation:
        ...
        textures_row.alert = True
        textures_row.label(
            text="Some images are set in UDIM mode. Please change them to single mode."
        )
...
def image_packing(layout):
    try:
        ...
        unpacked_images_row.scale_y = 0.9
        unpacked_images_row.label(text=f"UNPACKED IMAGES IN THE SCENE")
        unpacked_images_row = unpacked_images.row()
        unpacked_images_row.alert = True
        unpacked_images_row.scale_y = 0.9
        unpacked_images_row.label(
            text=f"Please activate File-External Data-Pack Resources (can take a while to change)"
        )
...
def unexpected_objects(layout):
    try:
        for obj in bpy.context.scene.objects:
            if (
                obj.name not in var_bodyparts.values()
                and obj.name not in var_garmenttypes.values()
                and obj.name != "Armature"
            ):
                raise KeyError
    ...
    for obj in bpy.context.scene.objects:
        if (
            ...
            strange_objects_row.scale_y = 0.9
            strange_objects_row.alert = True
            strange_objects_row.label(
                text=f"Unexpected object in the scene: [{obj.name}]. Please remove it"
            )
...
def metadata_selector(layout, panel_props):
    ...
    row0.operator(
        "object.activate_head",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_head"])
        else "NONE",
    )

    row1 = metadata_row.row()
    row1.operator(
        "object.activate_chest",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_chest"])
        else "NONE",
    )
    row1.operator(
        "object.activate_armstop",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_armsTop"])
        else "NONE",
    )
    row1.operator(
        "object.activate_legstop",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_legsTop"])
        else "NONE",
    )

    row2 = metadata_row.row()
    row2.operator(
        "object.activate_belly",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_belly"])
        else "NONE",
    )
    row2.operator(
        "object.activate_armsbottom",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_armsBottom"])
        else "NONE",
    )
    row2.operator(
        "object.activate_legsbottom",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_legsBottom"])
        else "NONE",
    )

    row3 = metadata_row.row()
    row3.operator(
        "object.activate_hips",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_hips"])
        else "NONE",
    )
    row3.operator(
        "object.activate_hands",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_hands"])
        else "NONE",
    )
    row3.operator(
        "object.activate_feet",
        icon="CHECKMARK"
        if bpy.context.scene.objects.get(var_bodyparts["var_feet"])
        else "NONE",
    )
    ...
    if panel_props.bodypartsBool:
        ...
        row4.prop(
            panel_props,
            "bodypartsBool",
            text="I have checked that the visible body parts are correct",
        )
    else:
        row4.alert = True
        row4.prop(
...
def gender_toggle(layout, panel_props):
    ...
    gender_toggle.prop(
        panel_props, "genderBool", toggle=True, invert_checkbox=True, text="Male"
    )


def style_toggle(layout, panel_props):
    style_toggle_box = layout.box()
    style_toggle = style_toggle_box.row(align=True)
    style_toggle.scale_y = 2
    style_toggle.prop(panel_props, "styleBool", toggle=True, text="Phr")
    style_toggle.prop(
        panel_props, "styleBool", toggle=True, invert_checkbox=True, text="Crt"
    )
...
def polygon_message(body_part, triangle_limit):
    try:
        ...
        if summa <= 0:
            ...
            return (
                False,
                f"{body_part} triangle count: {summa}/{triangle_limit}. Please reduce it to be able to export",
            )
...
def polycount_validation(layout):
    if (
        ...
        polycount_result, polycount_message = polygon_message(
            var_garmenttypes["var_top"], TOP_TRIANGLE_LIMIT
        )
        ...
        polycount_result, polycount_message = polygon_message(
            var_garmenttypes["var_bottom"], BOTTOM_TRIANGLE_LIMIT
        )
        ...
        polycount_result, polycount_message = polygon_message(
            var_garmenttypes["var_shoes"], SHOES_TRIANGLE_LIMIT
        )
        ...
        polycount_result, polycount_message = polygon_message(
            var_garmenttypes["var_acs"], ACCESSORIES_TRIANGLE_LIMIT
        )
    else:
        polycount_result, polycount_message = polygon_message(
            "Outfit", OUTFIT_TRIANGLE_LIMIT
        )

    if polycount_result:
        polygon_row = layout.box()
        polygon_row.alert = False
        polygon_row.scale_y = 0.9
...
def export_button(
    ...
    if (
        validate_required_objects(panel_props=panel_props)
        and polycount
        and panel_props.bodypartsBool
        and not unexpected_objects(layout=layout)
        and imagesize
        and imagepacking
        and not textures_validation(layout=layout)
        and armatureparent
        and transformations
    ):
        ...
        validation_row.label(
            text="Requirements are not satisfied. Fix them to enable export button"
        )
    ...
    for garment_types in var_garmenttypes.values():
        if check_existence(garment_types):
            try:
                ...
                armatureparent_row.label(
                    text=f"{garment_types} is not parented to the armature"
                )
    ...
    for garment_types in var_garmenttypes.values():
        if check_existence(garment_types):
            ...
            if (
                ...
                or obj.scale != Vector((1.0, 1.0, 1.0))
                or obj.rotation_euler != Euler((0.0, 0.0, 0.0))
            ):
    ...
    for garment_types in var_garmenttypes.values():
        if check_existence(garment_types):
            ...
            if (
                ...
                transformations_row.label(
                    text=f"{garment_types} has unapplied transformations"
                )
                var_transformations_applied = False
